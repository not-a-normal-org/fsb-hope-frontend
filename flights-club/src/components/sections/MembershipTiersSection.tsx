...
function TierCard({ tier }: { tier: MembershipTier }) {
  ...
  // Per-tier gradient config
  const circleSize = isElite ? 320 : 200;
  const gradientStops = isElite
    // Black: layered gold glow — more premium, wider, double-stop
    ? (x: number, y: number) =>
  ...
  return (
    ...
    >
      ...
      {isPopular && (
        <motion.div
          className="absolute top-6 right-6 px-3 py-1 rounded-full bg-accent-orange text-bg-primary text-xs font-bold uppercase tracking-wider z-10"
      ...
      {isElite && (
        <motion.div
          className="absolute top-6 right-6 px-3 py-1 rounded-full bg-accent-gold text-bg-primary text-xs font-bold uppercase tracking-wider z-10"
          initial={{ opacity: 0, y: -10 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3 }}
        >
